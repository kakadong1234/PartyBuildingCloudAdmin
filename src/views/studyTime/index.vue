...
<div class='app-container'>
  ...
  <el-table v-if="tableData.length" class='table' :data='tableData' stripe border @selection-change="handleSelectionChange" highlight-current-row @current-change="handleCurrentChange">
        ...
        <el-table-column prop='type' label='党员类型' width='180'></el-table-column>
        <el-table-column prop='studyTime' label='学习时长'></el-table-column>
    ...
    <div class='no_data_div' v-else >没有设置学习时长!</div>
 ...
 <script>
import { getStudyTimeList } from '@/api/studyTime'
...
export default {
  ...
  methods: {
    fetchData() {
      getStudyTimeList().then(response => {
    ...
    handleCurrentChange(val) {
      ...
      this.currentRow = val
      // TODO跳转 router
      this.$router.push({ path: '/studyTime/detail/' + this.currentRow.ID })
    },
    ...
    editRow(index, row, column, data) {
      console.log('edit')
      this.$router.push({ path: '/studyTime/edit/' +  row.ID})
    },
    detailRow(index, row, column, data) {
      console.log('detail')
      this.$router.push({ path: '/studyTime/detail/' +  row.ID})
    }
  }
}
</script>
